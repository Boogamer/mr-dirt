...
module.exports = {
    ...
    _removeRole(message, guildMember, role) {
        ...
        if(message.member.roles.highest.comparePositionTo(role) <= 0) {
            return message.channel.send(`Vous n'avez pas la permission de supprimer le rôle ${role} à ${guildMember}.`);
        }
        if (role.permissions.has("ADMINISTRATOR")) {
            return message.channel.send(`Vous n'avez pas la permission de supprimer le rôle ${role} à ${guildMember}.`);
        }
        guildMember.roles.remove(role)
            .then(m => message.channel.send(`${guildMember} n'a plus le rôle ${role}.`))
